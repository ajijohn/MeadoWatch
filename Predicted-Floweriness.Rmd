...
#Functions to predict probability of flowering, flowering richness
```{r}
...
This chunk of code estimates the probability of observing flowering of focal species along each MeadoWatch trail (RL and GB) for each year, between March 15 and October 15 (7 months). This is done in the following way:
...
for(i in 1:nyrs){ #each year
    ...
    outputRL_yr <- outputboth; outputGB_yr <- outputboth
    ...
    for(j in 1:2){ #each trail
        ...
        for(k in 1:length(spp_trl)){
            ...
            if(j==1){
              outputGB_yr <- cbind(outputGB_yr, ptrailf)
              dimnames(outputGB_yr)[[2]][k+2] <- spp_trl[k]}
            if(j==2){
              outputRL_yr <- cbind(outputRL_yr, ptrailf)
              dimnames(outputRL_yr)[[2]][k+2] <- spp_trl[k]}
        }
    if(j==1){outputGB <- rbind(outputGB,outputGB_yr)}
    if(j==2){outputRL <- rbind(outputRL,outputRL_yr)}
    }
}

#Calculate flowering indexes
#probability all focal species flower
allflGB <- apply(outputGB[,3:dim(outputGB)[2]], 1, prod)
allflRL <- apply(outputRL[,3:dim(outputRL)[2]], 1, prod)

#probability any focal species flowers
pnoGBa <- 1-outputGB[,3:dim(outputGB)[2]]
pnoRLa <- 1-outputRL[,3:dim(outputRL)[2]]
anyflGB <- 1-apply(pnoGBa, 1, prod)
anyflRL <- 1-apply(pnoRLa, 1, prod)

#append
outputGB <- cbind(outputGB, anyflGB,allflGB)
outputRL <- cbind(outputRL, anyflRL,allflRL)

#Examine output
#TODO why is ERGR 2015 acting odd?
head(outputGB)
head(outputRL)

```

#Make a representative graph **IN PROGRESS**
```{r}
#Examine output
#TODO why is ERGR 2015 acting odd?
head(outputGB)
...
#RL - representative graph - 2015, 2019
pltyrs <- c(2015, 2019)
par(mfrow=c(2,2), omi=c(0,0,0,0), mai=c(0.5,0.4,0.4,0.2), 
    tck=-0.01, mgp=c(1.25,0.25,0), xpd=TRUE)

for(i in 1:length(pltyrs)){
  yrdat <- outputRL[outputRL[,1]==pltyrs[i],]
  plot(yrdat[,2],yrdat[,10], type="l", col="pink", lwd=2, 
       xaxt="n", xlab="DOY", ylab="probability", 
       main=paste("Any Focal",pltyrs[i], sep=" "))
  text(c(105,135,165,195,225,255,285),-0.1,
       labels=c("Apr","May","Jun","Jul","Aug","Sep","Oct"))

  plot(yrdat[,2],yrdat[,11], type="l", col="pink", lwd=2, 
       xaxt="n",xlab="DOY", ylab="probability", 
       main=paste("All Focal",pltyrs[i], sep=" "))
  text(c(105,135,165,195,225,255,285),-0.1,
       labels=c("Apr","May","Jun","Jul","Aug","Sep","Oct"))
