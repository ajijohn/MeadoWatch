...
#Graph meadow level species richness by year / trail (**In Progress**)
...
layout(matrix(1:(nyrs*2), ncol=2))
par(omi=c(0,0,0,0), mai=c(0.4,0.4,0.4,0.3), tck=-0.02,mgp=c(1.25,0.5,0))

#for loop
for(i in 1:nyrs){
  for(j in 1:2){
    if(yrs[i]<=2014 & trails=="GB"){next}
    
    #calculate peak flowering - from mean snowmelt
    psp <- pars_spp_peak
    
    #extract year, trail specific range parameters
    psr <- pars_spp_range
    psr_yt <- psr[psr$year==yrs[i] & psr$site==trails[j],]
    
    #extract year, plot specific max parameters
    psm <- pars_spp_max
    psm_yt <- psm[psm$year==yrs[i] & psm$site==trails[j],]
    
    #species
    spp <- psm_yt$species
    
    #for loop plotting species
...
#Other tidbits to add

##QA/QC volunteer vs. expert values

##Output AIC values for plot level - null vs. pheno curve

##Output AIC values for all plot specific parameters vs model
