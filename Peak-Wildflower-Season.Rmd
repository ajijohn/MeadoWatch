...
#Calculate and filter according to observer effort
...
3. Calculate closest (in time) yes / no observation
...
#I'm doing this by for loop but this is inefficient. any ideas?
...
for(i in 1:dim(PhenoSite_Focal)[1]){
  ...
  compdat <- PhenoSite_Focal[PhenoSite_Focal$Year==yr&
                               PhenoSite_Focal$Site_Code==sc&
                               PhenoSite_Focal$Species==sp,]
...
#plot per species / trail
Transects <- unique(PhenoSite_Focal$Transect.x)

for(i in 1:2){
  TransectDat <- PhenoSite_Focal[PhenoSite_FOcal$Transect.x==Transects[i],]
  par(mforw=c(3,3), omi=c(0,0,0,0), mai=c(0.5,0.5,0.4,0.4),
      tck=-0.02, mgp=c(1.25,0.5,0))
  for(j in 1:length(Species)){
    TransectSpDat <- TransectDat[TransectDat$Species==species[j],]
  }
}
