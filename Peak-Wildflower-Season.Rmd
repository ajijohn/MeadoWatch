...
>>>>>>> 5cb1190edb7cead8c7076aef276ed8e34c6c9960
...
### Plots the predicted peak flowering day of the year (DOY) of each species by year/plot id. X-axis is the years, Y-axis is DOY, and colors indicate all the species. 
...
=======
### Alternative plot to visualize outlieers,  Site level (RL or GB) is on alternative y-axis, X-axis DOY, and Y-axis the focal species. We see the outliers are closer to start of the y-axis scale or towards the end. Columns here are years. SDD gardient can be ignored ( no variation)
>>>>>>> 5cb1190edb7cead8c7076aef276ed8e34c6c9960
```{r}
...
=======
# Augmenting the analysis to do pairwise comparison of abservations for finding differences to track outliers

## Observations by trail / year / plot / species - Sanity check on nuber of observations 
```{r}
PhenoDat %>% 
...
tallyObservations_typs <- PhenoDat %>% 
  group_by(Transect,Year,Site_Code,Species)%>% 
  tally() %>% as.data.frame()
```

# Below plot shows the variability of observations - notice a perfect cluster of # of observationsin 2015 Reflections lake. Also, what we see is the low number of observations for soe of the plots
>>>>>>> 5cb1190edb7cead8c7076aef276ed8e34c6c9960
```{r}
...
Imagine a specific query : Species =='ANOC' & Year=='2013' & Site_Code =='RL9'

? group_by orders the rows by given set of fields (in order)
...
PhenoDat %>% 
  filter (Flower == 1 & Species %in% species) %>% 
  mutate (doy = lubridate::yday(as.Date(Date, "%m/%d/%Y"))  ) %>% 
  group_by(Transect,Year,Site_Code,Species) %>% 
  mutate(pdiff = doy -     lag(doy, default = doy[1])) %>%
  filter (pdiff > 7) %>%
