...
Another option is to plot AIC null vs. AIC alt of all combos, and the 1:1 line. This should show most if not all AIC alt are less than AIC null. Could color code / change shapes depending on years / species
```{r}
...
#Code Chunks **In Progress**


#Outliers (AJI - 10/26)
##plots with outliers
##limited sampling early / late
## Definitely species with peak flowering dates that are outside conventional range
```{r}
plot_pars_years <- read_csv("data/PerPlotCurves.csv") 
plot_pars_years %>% ggplot() + geom_point(aes(as.factor(year),peak,color=species)) + facet_grid(.~plot) + theme_minimal() + theme(axis.text.x = element_text(angle = 90,hjust = 5)) +
  ...
  labs(x=" Year", y= "Peak flowering (DOY)", color="Species")

ggsave("figs/sanity_check.png",width = 70, height = 20, units = "cm")

``` 
Better plot, but it is at Site level (RL or GB)
```{r}

plot_pars_years %>% mutate(site= str_sub(plot_pars_years$plot,1,2)) %>% ggplot( aes(x = peak, y = species,fill=SDD)) +
   geom_jitter() +
...
```
# Pairwise comparison of abservations for finding differences to track outliers

Observations by trail / year / plot / species
```{r}

PhenoDat %>% group_by(Transect,Year,Site_Code,Species) %>% tally()
```

Determine all pairwise differences in dates between all yes observations in each trail / year / plot / species combo, and identify any observation where the minimum value is > 7

```{r}

PhenoDat %>% filter (Flower == 1) %>% mutate (doy = lubridate::yday(as.Date(Date, "%m/%d/%Y"))  ) %>% group_by(Transect,Year,Site_Code,Species) %>% mutate(pdiff = doy -     lag(doy, default = doy[1])) %>% filter (pdiff > 7) %>% ggplot( aes(x = Site_Code, y = Species,fill=pdiff)) +
   geom_jitter() +
...
```
Filter it only by focal species

```{r}

PhenoDat %>% filter (Flower == 1 & Species %in% species) %>% mutate (doy = lubridate::yday(as.Date(Date, "%m/%d/%Y"))  ) %>% group_by(Transect,Year,Site_Code,Species) %>% mutate(pdiff = doy -     lag(doy, default = doy[1])) %>% filter (pdiff > 7) %>% ggplot( aes(x = Site_Code, y = Species,fill=pdiff)) +
   geom_jitter() +
    facet_grid(Transect~Year)    +
  scale_fill_viridis_c(name = "Difference in DOY") +
    labs(title = 'Pairwise differences',
...
Plot by RL only, looks like 128 observations for RL (no jitter)

```{r}
PhenoDat %>% filter (Transect== 'Reflection Lakes' & Flower == 1 & Species %in% species) %>% mutate (doy = lubridate::yday(as.Date(Date, "%m/%d/%Y"))  ) %>% group_by(Transect,Year,Site_Code,Species) %>% mutate(pdiff = doy -     lag(doy, default = doy[1])) %>% filter (pdiff > 7) %>% ggplot( aes(x = Site_Code, y = Species,fill=pdiff)) +
   geom_point() +
    facet_grid(Transect~Year)    +
  scale_fill_viridis_c(name = "Difference in DOY") +
    labs(title = 'Pairwise differences',
...
Plot by RL only, looks like 128 observations for RL (with jitter)
```{r}
PhenoDat %>% filter (Transect== 'Reflection Lakes' & Flower == 1 & Species %in% species) %>% mutate (doy = lubridate::yday(as.Date(Date, "%m/%d/%Y"))  ) %>% group_by(Transect,Year,Site_Code,Species) %>% mutate(pdiff = doy -     lag(doy, default = doy[1])) %>% filter (pdiff > 7) %>% ggplot( aes(x = Site_Code, y = Species,fill=pdiff)) +
   geom_jitter() +
    facet_grid(Transect~Year)    +
