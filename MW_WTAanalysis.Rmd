...
#Another figure - by snow melt
```{r}
SDDdat <- read.csv("output/MeadowSDD.csv", header=TRUE)
yearavgs <- tapply(SDDdat$SDD, list(SDDdat$Year, SDDdat$Transect), mean)

#Now add mean month, mean visitation
...
This code chunk runs random forests on sentiments - relating total sentiment to the individual sentiments (except for bugs - only 40 records in GB). Two approaches to deal with missing values.
1. Impute missing values
2. Assume NA's are 3 (neutral opinion)
```{r}
...
#plot importance
par(mfrow=c(1,2))
varImpPlot(bag.sentiments.i, pch=21, bg="yellowgreen")
varImpPlot(bag.sentiments.n, pch=21, bg="yellowgreen")


#Plot a few scatter plots
par(mfrow=c(1,3), pty="s", omi=c(0,0,0,0), mai=c(0.5,0.5,0.3,0.3),
    tck=-0.02, mgp=c(1.0,0.3,0))

plot(jitter(sentiments$wildflower_s), jitter(sentiments$total_s),
     xlim=c(0,5), ylim=c(0,5), cex=1.5, 
     xlab="Wildflowers", ylab="Total Sentiment", pch=21, bg="magenta")
plot(jitter(sentiments$view_s), jitter(sentiments$total_s),
     xlim=c(0,5), ylim=c(0,5), cex=1.5,
     xlab="Views", ylab="Total Sentiment", pch=21, bg="lightblue")
plot(jitter(sentiments$wild_s), jitter(sentiments$total_s),
     xlim=c(0,5), ylim=c(0,5), cex=1.5,
     xlab="Wildlife", ylab="Total Sentiment", pch=21, bg="brown")

```

#Merge WTA data with MeadoWatch predictions of the flowering season and flowering richness.
```{r}
# Add DOY to WTA data
...
# Preliminary analysis exploring sentiment scores (ordinal regression)
```{r}
...
test_ord_flseas <- polr(total_s2 ~ as.factor(flseason), data=WTA_MW_dat)
...
anova(test_ord_null,test_ord_flwr,test_ord_rich,test_ord_flseas,
      test_ord_yr,test_ord_yrflwr,test_ord_yrrich)
AIC(test_ord_null,test_ord_flwr,test_ord_rich,test_ord_flseas,
      test_ord_yr,test_ord_yrflwr,test_ord_yrrich)
summary(test_ord_rich)
summary(test_ord_flseason)
