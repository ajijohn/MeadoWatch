...
Yeardat$NPS <- c(rep(NA, times=5),((NPS_monthvisit*30)-15))
#Yeardat$NPS <- c(rep(NA, times=5),NPS_monthvisit)
...
for(i in 1:length(yrs)){
  ...
  points(yrvisits$month_num, yrvisits$n/sum(yrvisits$n), 
         cex=ptsize, pch=21, bg=SDDcols[i])
...
for(i in 1:(dim(sentiments_recode)[2]-1)){
  ...
  #find sample size
  sampsize <- tapply(tot,ind,length); print(sampsize)
