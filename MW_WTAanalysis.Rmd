...
SDDdat <- read.csv("output/MeadowSDD.csv", header=TRUE) #TODO - SNOTEL?
...
for(i in 1:2){
  ...
  title(paste("Peak Flowering",trls[i],sep=": "))
  text(c(4,5,6,7,8,9,10),-0.08, xpd=NA, 
  ...
  for(j in 1:length(yrs)){
    yrvisits <- MW_flowers[MW_flowers$year==yrs[j]&MW_flowers$trail==trls[i],]
    ifelse(j<7, ptsize<-1.25, ptsize<-2)
...
for(i in 1:2){
  plot(c(6,1), type="n", xaxt="n", xlim=c(3.5,10.5), ylim=c(0,0.6),
       xlab="Month", ylab="Proportion")
  title(paste("WTA Trip Reports",trls[i],sep=": "))
...
text(c(4,5,6,7,8,9,10), -0.05, xpd=NA, 
      labels=c("Apr","May","June","July","Aug","Sep","Oct"))
legend(x="topright", legend=c("Average climate year", "CC analog year"), 
...
text(c(135, 165, 195), 175, xpd=NA, 
      labels=c("May","Jun","Jul"))
text(110, c(195, 215), xpd=NA, srt=90, labels=c("Jul","Aug"))
mtext("f)", side=3, line=-1, cex=0.65, adj=0.025)
cls <- c("magenta","yellowgreen","chocolate4","lightblue")

legend(x="bottomright", legend=c("MW","WTA","NPS"),
       lty=1, col=cls, pch=21, pt.bg=cls, pt.cex=1.5, cex=0.75)

#tests
Flwrtest <- lm(Yeardat$MW~Yeardat$SDD); print(summary(Flwrtest))
WTAtest <- lm(Yeardat$WTA~Yeardat$SDD); print(summary(WTAtest))
NPStest <- lm(Yeardat$NPS~Yeardat$SDD); print(summary(NPStest))

#add lines, but only for reach of data
...
abline(0,1, lwd=2, col="lightblue")
text(202,208,labels="snow melt", adj=0.5, srt=45, cex=0.65)
...
#Objective 2 Figure raw correlations
...
#Objective 2: recode sentiments, make figure, 1-way ANOVA
```{r fig1, fig.height=3, fig.width=8, fig.align="center"}
...
sentiments_recode[is.na(sentiments_recode)==TRUE] <- "N.A."
...
for(i in 1:(dim(sentiments_recode)[2]-1)){
  ...
  if(dimnames(sentiments)[[2]][i] %in% noplot == TRUE){next}
  plot(tot~ind, ylim=c(0.5,5.5), xlab = "Individual Sentiment",
       ylab = "Total Sentiment", 
       col=c("grey","yellow","chartreuse4","royalblue"))
  title(ttls[i])
  
  #Add NAS
  propnotext <- 100*round(length(ind[ind=="N.A."]) / length(ind),2)
  
  #add to graph
  mtext(paste(propnotext, "%", sep=""),
        line=-1, side=3, adj=0.06, cex=0.65)
...
#Total analysis
alltest <- lm(total.s~bug.s+trail.s+weather.s+view.s+wildflower.s,
              data=sentiments_recode)
print(anova(alltest))
print(TukeyHSD(aov(alltest)))

```

#Objective 2: Random Forests with sentiments
This code chunk uses random forests to explore how total sentiment is related to individual sentiments. We exclude crowding and wildlife (low irr). Three approaches are used
1. Use the full dataset, with recoded individual sentiments (NA, Neg, Neutral, Pos)
2. Use the full dataset, with quantitative sentiments, dealing with NAs as follows
    +recode them as 3 (neutral) - see TukeyHSD for justification
    +replace them with the mean sentiment of non NA values
    +impute them
3. Use a reduced dataset (reduced to those who experienced wildflowers). Deal with NAs here as follows
    +recode them as 3 (neutral) - see TukeyHSD for justification
    +replace them with the mean sentiment of non NA values
    +impute them
4. Create some plots to show these various results
...
#Create 3 datasets to deal with NAs (NA=3, NA=mean, imputed)
sentiments_NA3 <- sentiments
sentiments_NA3[is.na(sentiments_NA3)==TRUE] <- 3
sentiments_NAmn <- sentiments
for(i in 1:7){
  sentiments_NAmn[is.na(sentiments_NAmn[,i])==TRUE,i]<-
                  mean(na.omit(sentiments_NAmn[,i]))
}
sentiments_imputed <- rfImpute(total.s ~ ., sentiments)
...
## FULL DATA SET, REPLACE NA's 3 options
# Replace NAs with 3
rf.NA3 <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_NA3,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
...
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.NA3, pch=21, bg="yellowgreen", main="NA=3")

# Replace NAs with means
rf.NAmn <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_NAmn,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
#print, plot results
print(rf.NAmn, type=2)
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.NAmn, pch=21, bg="yellowgreen", main="NA=mean")

# Impute NAs
rf.imputed <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_imputed,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
#print, plot results
print(rf.imputed, type=2)
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.imputed, pch=21, bg="yellowgreen", main="NA=imputed")

## Rerun analyses with reduced dataset
# Now reduce data set to wildflower only
sentiments_wf_NA3 <- sentiments_NA3[is.na(sentiments$wildflower.s)==FALSE,]
sentiments_wf_NAmn <- sentiments_NAmn[is.na(sentiments$wildflower.s)==FALSE,]

#need to reimpute NAS
sentiments_wf <- sentiments[is.na(sentiments$wildflower.s)==FALSE,]
sentiments_wf_imputed <- rfImpute(total.s ~ ., sentiments_wf)

#Check: how many NAs in other sentiments?
for(i in 1:7){
  typ <- dimnames(sentiments_wf)[[2]][i]
  propNA <- round(sum(is.na(sentiments_wf[,i])) / dim(sentiments_wf)[1], 3)
  print(paste(typ,": NA sentiments = ",propNA, sep=""))
}

#Now random forests with reduced datasets
# NA = 3
rf.NA3.wf <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_wf_NA3,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
# print, plot results
print(rf.NA3.wf, type=2)
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.NA3.wf, pch=21, bg="yellowgreen", main="reduced: NA=3")

# NA = mean
rf.NAmn.wf <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_wf_NAmn,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
# print, plot results
print(rf.NAmn.wf, type=2)
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.NAmn.wf, pch=21, bg="yellowgreen", main="reduced: NA=mn")

# Impute NAs
rf.imputed.wf <-randomForest(total.s ~ bug.s + trail.s + weather.s + view.s +
                        wildflower.s, data=sentiments_wf_imputed,
                        importance=TRUE, keep.inbag = TRUE, ntree=500)
# print, plot results
print(rf.imputed.wf, type=2)
par(mfrow=c(1,2), omi=c(0,0,0,0), mai=c(0.3,0.3,0.3,0.25),
    mgp=c(1.0,0.25,0), tck=-0.01, cex.axis=0.8)
varImpPlot(rf.imputed.wf, pch=21, bg="yellowgreen", main="reduced: NA impute")
...
rf.names <- dimnames(rf.imp)[[1]]
rf.names <- substr(rf.names, 1, (nchar(rf.names)-2))

dotchart(rf.imp[,1], labels = rf.names, xlim = c(0,1.1*max(rf.imp[,1])),
         pch=21, bg="darkgreen", xlab = "Importance (MSE)", pt.cex = 2)
...
# Objective 3: Ordinal regressions
...
ordr_s2_null <- polr(total.s2~1, data=WTA_MW_dat)
ordr_s2_yr <- polr(total.s2~as.factor(year), data=WTA_MW_dat)
ordr_s2_rch <- polr(total.s2~rich, data=WTA_MW_dat)
ordr_s2_trl <- polr(total.s2~as.factor(trail), data=WTA_MW_dat)
ordr_s2_yrrch <- polr(total.s2~rich+as.factor(year), data=WTA_MW_dat)
ordr_s2_yrtrl <- polr(total.s2~as.factor(year)+as.factor(trail),
                      data=WTA_MW_dat)
ordr_s2_trlrch <- polr(total.s2~rich+as.factor(trail), data=WTA_MW_dat)
ordr_s2_yrxrch <- polr(total.s2~rich*as.factor(year), data=WTA_MW_dat)
ordr_s2_yrxtrl <- polr(total.s2~as.factor(year)*as.factor(trail),
                      data=WTA_MW_dat)
ordr_s2_trlxrch <- polr(total.s2~rich*as.factor(trail), data=WTA_MW_dat)

#various tests
anova(ordr_s2_rch, ordr_s2_null)
anova(ordr_s2_yrrch, ordr_s2_yr)
anova(ordr_s2_trlrch, ordr_s2_trl)
AIC(ordr_s2_null, ordr_s2_yr, ordr_s2_rch, ordr_s2_trl, 
...
ordf_s2_null <- polr(total.s2~1, data=WTA_MW_dat)
ordf_s2_yr <- polr(total.s2~as.factor(year), data=WTA_MW_dat)
ordf_s2_fl <- polr(total.s2~as.factor(flseason), data=WTA_MW_dat)
ordf_s2_trl <- polr(total.s2~as.factor(trail), data=WTA_MW_dat)
ordf_s2_yrfl <- polr(total.s2~as.factor(flseason) + 
                        as.factor(year), data=WTA_MW_dat)
ordf_s2_yrtrl <- polr(total.s2~as.factor(year)+as.factor(trail),
                      data=WTA_MW_dat)
ordf_s2_trlfl <- polr(total.s2~as.factor(flseason)+as.factor(trail), 
                       data=WTA_MW_dat)
ordf_s2_yrxfl <- polr(total.s2~as.factor(flseason)*as.factor(year),
                      data=WTA_MW_dat)
ordf_s2_yrxtrl <- polr(total.s2~as.factor(year)*as.factor(trail),
                      data=WTA_MW_dat)
ordf_s2_trlxfl <- polr(total.s2~as.factor(flseason)*as.factor(trail), 
                       data=WTA_MW_dat)
#various tests
anova(ordf_s2_fl, ordf_s2_null)
anova(ordf_s2_yrfl, ordf_s2_yr)
anova(ordf_s2_trlfl, ordf_s2_trl)
AIC(ordf_s2_null, ordf_s2_yr, ordf_s2_fl, ordf_s2_trl, 
    ordf_s2_yrfl, ordf_s2_yrtrl, ordf_s2_trlfl, 
    ordf_s2_yrxfl, ordf_s2_yrxtrl, ordf_s2_trlxfl)

AIC(ordf_s2_null, ordf_s2_yr, ordf_s2_trl,
    ordf_s2_fl, ordr_s2_rch, 
    ordf_s2_yrfl, ordr_s2_yrrch, 
    ordf_s2_trlfl, ordr_s2_trlrch, 
    ordf_s2_yrtrl)
...
snt <- rep(NA, times=dim(WTA_MW_dat)[1])
snt[WTA_MW_dat$total.s2[]==1] <- "neg"
snt[WTA_MW_dat$total.s2[]==2] <- "neut"
snt[WTA_MW_dat$total.s2[]==3] <- "pos"
snt <- as.factor(snt)

boxplot(rch ~ snt + trl, xlab="", ylim=c(0,13), xaxt = "n",
        ylab = "Richness", at=c(1,2,3,5,6,7), cex.axis=0.75,
        col=c("yellowgreen","yellowgreen","yellowgreen",
              "lightblue","lightblue","lightblue"))
text(c(2, 6), c(-1.2, -1.2), labels=c("GB", "RL"), adj=0.5, xpd=NA)
text(c(1,2,3,5,6,7), c(12.8, 12.7, 12.8, 12.8, 12.7, 12.8),
     cex=0.6, labels=c("<=neut","pos","+pos", "<=neut","pos","+pos"),
     adj=0.5, xpd=NA)
...
boxplot(snt ~ fl + trl, ylab = "Sentiment", xlab="", ylim=c(1,5.5),
        ...
        col=c("yellowgreen","yellowgreen",
              "lightblue","lightblue"))
text(c(1.5, 4), c(0.5, 0.5), labels=c("GB", "RL"), adj=0.5, xpd=NA)
