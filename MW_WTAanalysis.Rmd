...
```{r}
#fig1, fig.height=6, fig.width=5, fig.align="center"}
# set option for making figure
pdf(file="output/figures/Fig2.pdf", width=5, height=6,
     title="")
...
yrcols <- c("navyblue","navyblue","lightblue","navyblue",
            "lightblue","lightblue","orange")
...
text(c(4,5,6,7,8,9,10), -0.05, xpd=NA, 
      labels=c("Apr","May","June","July","Aug","Sep","Oct"))
legend(x="topright", legend=c("Late snow", "Avg snow","Early snow"), 
       cex=0.75, pch=21, pt.bg=c("navyblue","lightblue","orange"), pt.cex=1.75)
...
#Objective 2: Figure 3
This chunk of code recodes individual sentiments are recoded into categorical variables.
...
sentiments_recode[is.na(sentiments_recode)==TRUE] <- "None"
...
1. Use the full dataset, with recoded individual sentiments (NA, Neg, Neutral, Pos). Note, this is what we report in the manuscript; results are in rf.recode
...
1. Plot bar graphs of individual sentiments (a-e)
...
for(i in 1:(dim(sentiments_recode)[2]-1)){
  ...
  title(ttls[i])
  mtext(lbl[i], side=3, line=-1.1, cex=0.8, adj=0.025)
  
  #Add NAS
  #propnotext <- 100*round(length(ind[ind=="None"]) / length(ind),2)
  
  #add to graph
  #mtext(paste(propnotext, "%", sep=""),
  #      line=-2, side=3, adj=0.1, cex=0.65)
...
snt2 <- snt
snt2[snt[]<=3] <- "neg-neut"
snt2[snt[]>3 & snt[] <=4] <- "pos"
snt2[snt[]>4] <- "pos+"
snt2 <- as.factor(snt2)


boxplot(rch ~ snt2 + trl, xlab="", ylim=c(0,13), xaxt = "n",
...
#Alternative Fig 3: USED FOR NOW
...
#Alternative Fig 3: continuous flowering richness (for reviewer response)
```{r fig1, fig.width=9, fig.height=3.0}



par(mfrow=c(1,1), omi=c(0,0,0,0), mai=c(0.4,0.4,0.35,0.3), 
    mgp=c(1.1,0.4,0), tck=-0.02)
boxplot(snt ~ rch + trl, ylim=c(1,5.5), cex.axis=0.5, 
        xlab="Flowering Richness", ylab = "Sentiment", 
        col = c(rep("yellowgreen",times=13), rep("lightblue",times=13)))
mtext("a)", side=3, line=0, cex=0.65, adj=0.025, xpd=NA)
