...
# Now make data frame of dates by SDD (*check this)
...
#Create year dat two, for analysis
...
#create color ramp
#minSDD <- min(YeardatRL$SDD); maxSDD <- max(YeardatRL$SDD)
#difSDD <- maxSDD-minSDD
#SDDramp <- (YeardatRL$SDD - minSDD) / difSDD
...
rch2 <- rch
rch2[rch[]<=2] <- "0-2"
rch2[rch[]>2 & rch[]<=6] <- "3-6"
rch2[rch[]>7] <- "7+"
rch2 <- as.factor(rch2)
snt <- as.numeric(WTA_MW_dat$total.s)
snt2 <- as.numeric(WTA_MW_dat$total.s2)

#by trail 

vioplot(snt ~ rch2 + trl, xlim=c(0,8), ylim=c(1,5),
        horizontal=FALSE, xaxt = "n", cex.axis=0.75, at=c(1,2,3,5,6,7),
        xlab="", ylab = "Sentiment", 
        col = c("magenta","magenta","magenta",
               "magenta4","magenta4","magenta4"))
text(c(1,2,3,5,6,7), c(rep(0.7, times=8)), adj=0.5, xpd=NA, cex=0.6,
     labels=c("<3","3-6","7+","<3","3-6","7+"))
text(5, 0.4, labels=c("Flowering Richness"), adj=0.5, xpd=NA)
mtext("a)", side=3, line=0, cex=0.65, adj=0.025, xpd=NA)
legend(x="bottomright", legend=c("GB","RL"),
       fill=c("magenta","magenta4"), cex=0.75)

#stripchart(snt ~ rch2 + trl, vertical = TRUE, at=c(1,2,3,4,6,7,8,9),
