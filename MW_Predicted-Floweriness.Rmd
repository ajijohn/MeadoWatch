...
1. *PerPlotCurves.csv*: includes peak, range and max parameters (for a unimodal curve describing the relationship between DOY and flowering probability), in each year, trail, plot in which species were observed. In other words, this model was separately fit to year-trail-plot-species specific data.

2. A second set of models were fit to each species data. This model assumed the same functional form - with the relationship between DOY and flowering probability in each plot determined by 4 parameters (in *SpeciesPeakRangeMax.csv*. In this model, however, the following was assumed:
...
3. *MW_SDDall.csv* includes observed and predicted snow disappearance data collected from each trail / plot.
...
Species_pars <- read.csv("data/SpeciesPeakRangeMax.csv", header=TRUE)
...
output_p <- c()
output_r <- c()
f_thresh <- 1.0 #between 0 and 1, modifies at what proportion of max flowering,
...
#write output
write.csv(output_p_fin, "data/FloweringProbabilities.csv", quote=FALSE,
          row.names=FALSE)
write.csv(output_r_fin, "data/FloweringRichness.csv", quote=FALSE,
          row.names=FALSE)
