...
for(i in 1:nyrs){ #each year
    ...
    for(j in 1:2){ #each trail
        ...
        spp_trl <- unique(PeakSDD_pars$species)
        ...
        for(k in 1:length(spp_trl)){
            ...
            #if zero, make mean
            if(length(max_yrtrl)==0){max_yrtrl <-
              mean(Max_pars$max[Max_pars$species==spp_trl[k]])}
            if(length(rng_yrtrl)==0){rng_yrtrl <-
              mean(Range_pars$range[Range_pars$species==spp_trl[k]])}   
